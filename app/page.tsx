...
>          
      ...
      <div className={`w-full transition-all duration-300 bg-slate-950 px-1 ${getWidthClass()}`}>
        ...
        {activePage === "staking" && (
            <section className="w-full max-w-full overflow-hidden px-1 md:px-0 space-y-6 animate-fade-in block">
              ...
              <div className="w-full bg-slate-900/40 border border-slate-800/60 p-4 rounded-2xl flex justify-between items-center gap-3">
                <div className="min-w-0 flex-1">
                  <h2 className="text-base font-black text-slate-100 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis">
                    ⚡ 거래소 실시간 스테이킹 현황
                  </h2>
                ...
                <div className="w-full space-y-6 block">
                  {stakingData.map((group, idx) => (
                    <div key={idx} className="w-full bg-slate-900/40 border border-slate-800/60 p-4 md:p-5 rounded-2xl block">
                      ...
                      {/* ✅ 하단 제어 바 상태(viewMode)와 실제 화면 크기(isMobile)를 모두 체크하는 조건문 조건문 주입 */}
                      {viewMode === "mobile" || viewMode === "tablet" || isMobile ? (
                        
                        /* 📱 [모바일 뷰]: 길쭉한 가로 한 줄 리스트 형태 고정 */
